...
public class PlantStraightProjectile : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(original_pos, transform.position) >= tile_counter)
        {
            ...
            {
                GameObject tmp_path;
                tmp_path = Instantiate(element_path, new Vector3(transform.position.x, 0.2f, transform.position.z), transform.rotation);
