...
public class FireStraightProjectile : MonoBehaviourPun, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (photonView.IsMine)
        {
            if (distance >= tile_counter)
            {
                ...
                {
                    GameObject tmp_path;
                    tmp_path = PhotonNetwork.Instantiate("FireStraightPath", new Vector3(transform.position.x, 0.6f, transform.position.z), transform.rotation, 0);
                ...
                else if (tile_counter > 1)
                {
                    ...
                    tmp_path = PhotonNetwork.Instantiate("FireStraightPath", tmp_path_position, transform.rotation, 0);
                }
