    ...
    void Update()
    {
        ...
        if (photonView.IsMine)
        {
            if (distance >= tile_counter)
            {
                ...
                {
                    GameObject tmp_path;
                    tmp_path = PhotonNetwork.Instantiate("PlantStraightPath", new Vector3(transform.position.x, 0.2f, transform.position.z), transform.rotation);
                ...
                else if (tile_counter > 1)
                {
                    ...
                    tmp_path_position = new Vector3((first_path_go_pos.x + ((tile_counter - 1) * projectile_direction_normalized.x)), 0.2f, (first_path_go_pos.z + ((tile_counter - 1) * projectile_direction_normalized.z)));
                    tmp_path = PhotonNetwork.Instantiate("PlantStraightPath", tmp_path_position, transform.rotation);
                }
