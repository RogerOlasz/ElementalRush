...
public class IceStraightProjectile : MonoBehaviourPun, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (photonView.IsMine)
        {
            if (distance >= tile_counter)
            {
                ...
                {
                    GameObject tmp_path;
                    tmp_path = PhotonNetwork.Instantiate("IceStraightPath", new Vector3(transform.position.x, 0f, transform.position.z), transform.rotation);
                ...
                else if (tile_counter > 1)
                {
                    ...
                    tmp_path = PhotonNetwork.Instantiate("IceStraightPath", tmp_path_position, transform.rotation);
                }
